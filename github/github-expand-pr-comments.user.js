...
// @namespace   https://github.com/ze0n
...
(function() {
    ...
    const hideResolvedButtonText = "Hide Resolved";
    ...
    {
        var allComments = document.querySelectorAll("turbo-frame [id^=review-thread-or-comment-id-]");

        let allCommentsArray = [];
        allComments.forEach((p)=>allCommentsArray.push(p));

        allCommentsArray.filter((c)=>
                                {
            let sel = c.querySelector("details:nth-child(1) > summary:nth-child(1) > div:nth-child(1) > span.btn-link");

            if(sel != null){
                return sel.textContent.trim() === "Show resolved"
            }else{
                return false;
            }
    ...
    function hideResolvedButtonClickFunction() {
